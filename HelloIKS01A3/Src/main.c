...
{
	printf("> Alarmanlage\r\n\r\n");
	int32_t baseline_acc[3];
	int32_t changed_acc[3];
	
	int32_t delta_acc[3];
	for(;;)
	{
		accelerometer.get_a_axes(baseline_acc);
		delay(1);
		accelerometer.get_a_axes(changed_acc);
		
		for(int32_t i = 0; i < 3; i++)
		{
			delta_acc[i] = abs(changed_acc[i] - baseline_acc[i]);
		}
		
		for(int32_t i = 0; i < 16; i++)
		{
			int temp = ACC_SENS * (i+1);
			int mask = (delta_acc[0] > temp || delta_acc[1] > temp || delta_acc[2] > temp) ? 0 : 16;
			if(i > 7)
			{
				GPIOE->BSRR = (1 << ((i - 8) + mask));
			}
			else
			{
				GPIOD->BSRR = (1 << (i + mask));
		...
		printf("X: %d, Y: %d, Z: %d\r\n", delta_acc[0], delta_acc[1], delta_acc[2]);
	}
}
