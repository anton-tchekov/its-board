...
static int _get_precedence(TokenType tt);
static int _parser_function(NanoC *n, const char *name, u32 len);
static int _parser_block(NanoC *n);
static int _parser_statement(NanoC *n);
static int _parser_substmt(NanoC *n, TokenType term);
static int _parser_fncall(NanoC *n, int i);
static int _parser_fnimpl(NanoC *n, const char *name, u32 len);
static int _parser_expression(NanoC *n);
static int _parser_type_annotation(NanoC *n);
static void _handle_break(NanoC *n, u32 prev, u32 addr_break);
static void _handle_continue(NanoC *n, u32 prev, u32 addr_continue);

int nanoc_compile(NanoC *n, const char *src, u8 *out, u8 *strings, int string_offset)
{
	int i;
	u32 idx_main, len;
	const char *name;
	...
	for(i = 0; i < (int)n->Parser.Functions.Count; ++i)
	{
...
static int token_to_instr(int token)
{
	switch(token)
	{
	case TT_U_MINUS:
		return INSTR_U_MINUS;
...
static int _parser_substmt(NanoC *n, TokenType term)
{
	...
	for(;;)
	{
		...
		else
		{
			...
			if(at)
			{
				n->Parser.Output[n->Parser.Index++] = token_to_instr(at);
			}
